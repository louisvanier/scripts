class CharacterKlass
    CLASS_LIST = {
		...
		'barbarian' => ['berserker', 'totem warrior', 'battlerager', 'ancestral guardian', 'storm herald', 'zealot', 'beast', 'wild magic', 'juggernaut', 'giant', 'wild heart', 'world tree'],
		'bard' => ['lore', 'valor', 'glamour', 'swords', 'whispers', 'creation', 'eloquence', 'spirits', 'tragedy', 'dance', 'glamour'],
		'cleric' => ['knowledge', 'life', 'light', 'nature', 'tempest', 'trickery', 'war', 'death', 'arcana', 'ambition (psa)', 'solidarity (psa)', 'strength (psa)', 'zeal (psa)', 'forge', 'grave', 'order', 'peace', 'twilight', 'blood', 'moon'],
		'druid' => ['land', 'moon', 'dreams', 'shepherd', 'spores', 'stars', 'wildfire', 'blighted', 'sea'],
		'fighter' => ['battle master', 'champion', 'eldritch knight', 'purple dragon knight (banneret)', 'arcane archer', 'cavalier', 'samurai', 'echo knight', 'psi warrior', 'rune knight'],
		'monk' => ['shadow', 'four elements', 'open hand', 'long death', 'drunken master', 'kensei', 'sun soul', 'mercy', 'astral self', 'ascendant dragon', 'cobalt soul', 'elements'],
		'paladin' => ['devotion', 'ancients', 'vengeance', 'oathbreaker', 'crown', 'conquest', 'redemption', 'glory', 'watchers', 'open sea'],
		'ranger' => ['beast master', 'hunter', 'gloom stalker', 'horizon walker', 'monster slayer', 'fey wanderer', 'swarmkeeper', 'drakewarden'],
		'rogue' => ['arcane trickster', 'assassin', 'thief', 'inquisitive', 'mastermind', 'scout', 'swashbuckler', 'phantom', 'soulknife'],
		'sorcerer' => ['pyromancer (PSK)', 'divine soul', 'shadow', 'storm', 'runechild', 'lunar', 'aberrant', 'clockwork', 'draconic', 'wild magic'],
		'warlock' => ['archfey', 'fiend', 'great old one', 'undying', 'celestial', 'hexblade', 'fathomless', 'genie', 'undead'],
		'wizard' => ['abjurer', 'conjuration', 'diviner', 'enchantment', 'evoker', 'illusionist', 'necromancy', 'transmutation', 'war', 'chronurgy', 'graviturgy', 'bladesinging', 'scribes', 'blood magic'],
	}.freeze
	...
	class << self
		...
		CharacterKlass::CLASS_LIST.each do |cls, _|
			define_method("#{cls}".to_sym) do |**args|
				...
				if klass_cache[cache_key].nil?
					klass_cache[cache_key] = CharacterKlass.new(cls, klass_source)
				end
				return klass_cache[cache_key]
			end
		end
    ...
    def initialize(character_class, class_source = nil)
        ...
        @class_details = Scraper.instance.get_character_class(character_class)["class"].find { |klass| klass["source"] == class_source }
		...
		@class_details["classFeatures"].each do |f|
			...
			feature = KlassFeature.new(Scraper.instance.get_character_class(character_class)["classFeature"].find { |f| f["name"].downcase == feature_name}, nil)
			@class_features << feature
		end

		@subclasses = []
		load_subclasses(Scraper.instance.get_character_class(character_class)["subclass"], Scraper.instance.get_character_class(character_class)["subclassFeature"])
    ...
    def load_subclasses(raw_data_subclasses, raw_data_features)
		...
		filtered = raw_data_subclasses.reject { |phb_version| phb_version["classSource"] == "PHB" && raw_data_subclasses.any? {|xphb_version| xphb_version["classSource"] == "XPHB" && xphb_version["name"] == phb_version["name"] }}
...
class KlassFeature
	...
	attr_accessor :name, :level, :description, :other_features_referenced, :subclass, :bonus_action, :reaction, :action, :subclass_short_name, :extra_spells
